...
knitr::opts_knit$set(root.dir = "C:/Users/bryce/OneDrive/Documents/Classes/WR-674/Hayman_Fire_NDVI_Trace#2")
...
```
#How do remotely sensed snow cover metrics impact discharge in the same water year in Colorado?
...
```{r read-data, include=T,results='hide',warning=F,error=FALSE}
library(tidyverse) 
library(lubridate)
library(dataRetrieval) #for downloading USGS data
...
```{r plot-mean snow vs median q, echo=F}
q_snow_means <- merge(q_means,snow_means)

ggplot(q_snow_means, aes(x=mean_ndsi,y=median_q,color=wtr_yr))+
  geom_point()
```
###Max Snow vs. Median Q
```{r plot-max snow vs mean q, echo=F}
ggplot(q_snow_means, aes(x=max_ndsi,y=median_q,color=wtr_yr))+
  geom_point()
```
